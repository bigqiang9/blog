...
class RecycleController extends AdminBaseController{
    ...
    public function delete(){
        $data=I('get.');
        if ($data['table_name'] == 'File') {
            $where = array($data['id_name']=>$data['id_number']);
            $info = M($data['table_name'])->where($where)->find();
            unlink('./'.$info['upload_path']);
        }
