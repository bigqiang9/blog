...
html;
                \Think\Log::write('---开始给站长发送通知邮件----->',$content);
                send_email($address,$nickname.'评论了 '.$title,$content);
                \Think\Log::write('---结束给站长发送通知邮件----->');
            }
        }
        // 给用户发送邮件通知
...
html;
                \Think\Log::write('---开始给用户发送邮件通知----->',$parent_content);
                send_email($parent_address,$nickname.'回复了 '.$title,$parent_content);
                \Think\Log::write('---结束给用户发送邮件通知----->');
            }
